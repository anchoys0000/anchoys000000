/* ============================================================
   ОТПРАВКА ЗАКАЗА ЧЕРЕЗ CLOUDFLARE WORKER
   Вызывается из index.html после оформления заказа.
   Настройки лежат в config.js (WORKER_URL).
   Токен Telegram НЕ хранится на сайте.

   Секретного ключа на сайте НЕТ: спрятать его на статике
   невозможно, а видимый ключ защищает слабее, чем Worker.
   Защита от спама — на стороне Worker (рейт-лимит по IP,
   honeypot-поле, проверка времени заполнения формы).
   ============================================================ */
